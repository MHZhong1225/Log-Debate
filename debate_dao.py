...
def parse_args():
    ...
    p.add_argument("--context_window", type=int, default=10, help="Number of preceding logs for LLM context")
    
    p.add_argument("--lambda_coeff", "-l", type=float, default=0.3, help="Weight for LLM risk score r_t (lambda)")
    p.add_argument("--decision_threshold", "-d", type=float, default=0.45, help="Final decision threshold (delta)")
    
    # 默认值 [0.1, 0.9] 意味着对非常自信的样本 (s_t < 0.1 或 s_t > 0.9) 跳过 LLM
    # p.add_argument("--uncertainty_lower", type=float, default=0.05, help="Lower bound of uncertainty (trigger LLM if s_t > this)")
    # p.add_argument("--uncertainty_upper", type=float, default=0.9, help="Upper bound of uncertainty (trigger LLM if s_t < this)")
    p.add_argument("--confidence_threshold", "-ct", type=float, default=0.9, 
                 help="Trigger LLM if max confidence is BELOW this threshold (e.g., 0.9)")
    
    return p.parse_args()

# def call_llm_for_debate_RCDL(target_log: str, context_logs: list[str], mlp1_score: float, tokenizer, model) -> str:
def call_llm_for_DaO(target_log: str, s_t: float, context_logs: list[str], tokenizer, model) -> str:
...
[Coco Filter Analysis]: 
The lightweight Coco filter has assessed this event with a coherence score of {s_t}. 
(A score near 1.0 means high coherence with context; a score near 0.0 means high conflict.)
...
def parse_llm_verdict(llm_output: str, default_score: float) -> float:
    """
...
def main():
    ...
    csv_path = f"results/Debate_outputs_{DATASET}.csv"
    with open(csv_path, mode="w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["global_idx", "target_log", "context_logs", "mlp1_score", "llm_output", "r_t", "p_t"])
    ...
    print(f"--- Running DAO Evaluation ---")
    ...
    with torch.no_grad():
        for i, ((log_vec, ctx_vec), labels) in enumerate(tqdm(test_loader, desc="RCDL Evaluation")):
            ...
            # Coco
            # _, logits_mlp1, _, _ ,_ = model_mlp1(log_vec, ctx_vec)
            ...
            softmax_probs = F.softmax(logits_mlp1, dim=1)
            anomaly_probs_mlp1 = softmax_probs[:, 1]
            final_anomaly_scores = anomaly_probs_mlp1.clone()
            confidence_score = torch.max(softmax_probs, dim=1).values
            trigger_mask = (confidence_score < args.confidence_threshold)
            ...
            if trigger_mask.any():
                ...
                for idx_in_batch in triggered_indices:
                    ...
                    llm_output = call_llm_for_DaO(target_log, s_t, context_logs, llm_tokenizer, llm_model)
                    print(llm_output)
                    r_t = parse_llm_verdict(llm_output, default_score=s_t)
                    # r_t = parse_risk_score(llm_output, default_score=s_t)
                    s_t_anomaly = 1.0 - s_t
                    # (p_t)
                    p_t = (args.lambda_coeff * r_t) + (1 - args.lambda_coeff) * s_t_anomaly
