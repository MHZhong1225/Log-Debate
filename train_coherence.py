...
EPOCHS      = args.epochs
LR          = args.lr
MODEL_PATH  = f"model/{DATASET}/{DATASET}_model.pt"
...
def eval_model(model, loader, device):
    ...
    return f1, acc, auroc, auprc, y_true_binary, y_pred, anomaly_probs
...
if __name__ == "__main__":
    ...
    for epoch in range(EPOCHS):
        ...
        val_f1, val_acc, val_auroc, val_auprc, _, _,_ = eval_model(model, val_loader, DEVICE)
        print(f"Epoch {epoch+1:02d}/{EPOCHS} | Loss: {avg_loss:.4f} | Val F1: {val_f1:.4f}")
        # print(f"Epoch {epoch+1:02d}/{EPOCHS} | Loss: {avg_loss:.4f} | Val F1: {val_f1:.4f} | Val AUROC: {val_auroc:.4f} | Val AUPRC: {val_auprc:.4f} | lr {scheduler.get_last_lr()}\n")
        
        print(f"clf: {loss_clf.item():.4f}, coh: {loss_coherence.item():.4f}")
    ...
    test_f1, test_acc, test_auroc, test_auprc, y_true, y_pred, confidence_scores = eval_model(model, test_loader, DEVICE)
